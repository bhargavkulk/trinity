...
int VM::run() {
    ...
    while (pc < bytecode.size()) {
        ...
        switch (read_op()) {
            ...
            case OP::JMP_TRUE: {
                i16 offset = static_cast<i16>(read_word());
                printf("%d\n", offset);
                pc += offset;
                break;
            }
...
void VM::write_byte(u8 byte) { bytecode.push_back(byte); }
...
void VM::write_op(OP op) { bytecode.push_back(static_cast<u8>(op)); }
...
void VM::undecl_vars(usize count) { vars_declared -= count; }

/*********************************************************************/

OP VM::read_op() { return static_cast<OP>(bytecode.at(pc++)); }

u8 VM::read_byte() { return bytecode.at(pc++); }
...
void VM::push(u64 num) { stack.push_back(num); }
...
u64 VM::peek(usize offset) { return stack.at(stack.size() - 1 - offset); }
...
void set_execution_log(FILE *file) { execution_log = file; }

#endif /* DEBUG_FLAG */

// Bhargav
usize VM::bytecode_len() { return bytecode.size(); }

void VM::patch_jump(usize offset) {
    i64 jump = bytecode.size() - offset - 2;
    if (jump > UINT16_MAX) {
        fprintf(stderr, "Too much code.\n");
        exit(1);
    }

    bytecode[offset] = static_cast<u8>((jump >> 8) & 0xff);
